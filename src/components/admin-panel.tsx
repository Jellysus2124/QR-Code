...
export function AdminPanel({ contributors, qrs }: Props) {
  ...
  const run = async (handler: () => Promise<{ ok: boolean; message: string }>) => {
    ...
    try {
      ...
      setError("Co loi xay ra. Thu lai.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onExportPdf = async () => {
    if (selectedCodes.length === 0) {
      setError("Vui long chon it nhat 1 QR de export.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(payload.message ?? "Export PDF that bai.");
        return;
      }
      ...
      setFeedback(`Da export PDF cho ${selectedCodes.length} QR.`);
    } catch {
      setError("Khong the tai file PDF. Thu lai.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-3 sm:grid-cols-3">
        <StatCard title="QR chua gan" value={availableCount} />
        <StatCard title="QR da gan" value={assignedCount} />
        <StatCard title="QR da su dung" value={usedCount} />
      </section>

      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        <h2 className="text-lg font-bold">Tao lo QR</h2>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        <h2 className="text-lg font-bold">Import contributor tu Google Form (CSV)</h2>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        <h2 className="text-lg font-bold">Gan QR cho contributor</h2>
        <form onSubmit={onAssign} className="mt-3 space-y-3">
          <select
            ...
            required
          >
            <option value="">Chon contributor</option>
            {contributors.map((item) => (
              <option key={item.id} value={item.email}>
                {item.email} ({item.donation_quantity} bo)
              </option>
            ))}
          </select>
          <textarea
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        <h2 className="text-lg font-bold">Export QR ra PDF A4</h2>
        <p className="mt-1 text-sm text-slate-600">
          Chon QR ben duoi, sau do bam Export de tai file in.
        </p>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        <h2 className="text-lg font-bold">Contributor</h2>
        <ul className="mt-3 space-y-2 text-sm">
          {contributors.map((item) => (
            <li key={item.id} className="rounded-lg border border-slate-200 px-3 py-2">
              <div className="font-semibold">{item.email}</div>
              <div className="text-slate-600">{item.full_name ?? "Chua cap nhat ten"}</div>
              <div className="text-slate-600">So bo da dong gop: {item.donation_quantity}</div>
            </li>
          ))}
        </ul>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        <h2 className="text-lg font-bold">QR moi nhat (chon de export)</h2>
        <ul className="mt-3 space-y-2 text-sm">
          {visibleQrs.map((item) => (
            <li key={item.id} className="rounded-lg border border-slate-200 px-3 py-2">
              <label className="flex cursor-pointer items-start gap-3">
                ...
                <span>
                  ...
                  <span className="block text-slate-600">Trang thai: {item.status}</span>
                  {item.assigned_email && (
                    <span className="block text-slate-600">Nguoi nhan thong bao: {item.assigned_email}</span>
                  )}
                </span>
              </label>
            </li>
